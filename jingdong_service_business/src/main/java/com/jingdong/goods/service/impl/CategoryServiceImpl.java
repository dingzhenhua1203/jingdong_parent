...
import com.jingdong.goods.dao.CategoryMapper;
...
public class CategoryServiceImpl implements CategoryService {
    ...
    public List<Map> findCategoryTree() {
        ...
        return recurseCategoryTree(categories, 0);
    }

    private List<Map> recurseCategoryTree(List<Category> categories, Integer parentId) {
        if (categories == null || categories.isEmpty()) {
            return null;
        }
        List<Map> result = new ArrayList<>();
        List<Category> match = categories.stream().filter(x -> x.getParentId().equals(parentId)).collect(Collectors.toList());
        ...
        match.forEach(x -> {
            ...
            item.put("menu", recurseCategoryTree(categories, x.getId()));
            result.add(item);
        });
        return result;
    }
